...
local FallbackFont = Font.new("rbxasset://fonts/families/GothamSSm.json")

local Fonts = {
    ["Gotham SSm"] = Font.new("rbxasset://fonts/families/GothamSSm.json"),
    ["Gotham Bold"] = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Bold),
    ["Gotham Medium"] = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium),
    ["Gotham"] = Font.new("rbxasset://fonts/families/GothamSSm.json"),
    ["Source Sans Bold"] = Font.new("rbxasset://fonts/families/SourceSansPro.json", Enum.FontWeight.Bold),
    ["Source Sans Semibold"] = Font.new("rbxasset://fonts/families/SourceSansPro.json", Enum.FontWeight.SemiBold),
    ["Source Sans"] = Font.new("rbxasset://fonts/families/SourceSansPro.json"),
    ["Arial Bold"] = Font.new("rbxasset://fonts/families/Arial.json", Enum.FontWeight.Bold),
    ["Arial"] = Font.new("rbxasset://fonts/families/Arial.json"),
    ["Legacy"] = Font.new("rbxasset://fonts/families/GothamSSm.json"),
    ["Roboto"] = Font.new("rbxasset://fonts/families/Roboto.json"),
    ["Roboto Mono"] = Font.new("rbxasset://fonts/families/RobotoMono.json"),
    ["Oswald"] = Font.new("rbxasset://fonts/families/Oswald.json"),
    ["Ubuntu"] = Font.new("rbxasset://fonts/families/Ubuntu.json"),
    ["Nunito"] = Font.new("rbxasset://fonts/families/Nunito.json"),
    ["Merriweather"] = Font.new("rbxasset://fonts/families/Merriweather.json"),
    ["Michroma"] = Font.new("rbxasset://fonts/families/Michroma.json"),
    ["Kalam"] = Font.new("rbxasset://fonts/families/Kalam.json"),
    ["Indie Flower"] = Font.new("rbxasset://fonts/families/IndieFlower.json"),
    ["Patrick Hand"] = Font.new("rbxasset://fonts/families/PatrickHand.json"),
    ["Permanent Marker"] = Font.new("rbxasset://fonts/families/PermanentMarker.json"),
    ["Luckiest Guy"] = Font.new("rbxasset://fonts/families/LuckiestGuy.json"),
    ["Fredoka One"] = Font.new("rbxasset://fonts/families/FredokaOne.json"),
    ["Bangers"] = Font.new("rbxasset://fonts/families/Bangers.json"),
    ["Titillium Web"] = Font.new("rbxasset://fonts/families/TitilliumWeb.json"),
    ["Code"] = Font.new("rbxasset://fonts/families/Code.json")
}

local CurrentFontName = "Fredoka One"
...
local function SetInterFont(instance, weight)
    ...
    local ok = pcall(function()
        instance.FontFace = CurrentFontFamily
    end)
    if not ok then
        pcall(function() instance.FontFace = FallbackFont end)
    end
end
...
function Library:GetTranslation(key)
    local entry = Library.Translations[key]
    if entry then
        return entry[CurrentLanguage] or entry["EN"] or key
    end
    return key
end

function Library:ForceLanguage(langCode)
...
function Library:CreateWindow(config)
    ...
    local languageDefault = ConfigData.Language or config.Language or "EN"
    CurrentLanguage = languageDefault
    ...
    WindowTitleLabel.Name = "WindowTitle"
    WindowTitleLabel.Size = UDim2.new(1, -16, 0, 16)
    WindowTitleLabel.Position = UDim2.new(0, 8, 0, 8)
    WindowTitleLabel.Text = titleText
    WindowTitleLabel.TextSize = 13
    SetInterFont(WindowTitleLabel, "Bold")
    WindowTitleLabel.TextXAlignment = Enum.TextXAlignment.Center
    WindowTitleLabel.TextTruncate = Enum.TextTruncate.AtEnd
    WindowTitleLabel.BackgroundTransparency = 1
    WindowTitleLabel.ZIndex = 13
    WindowTitleLabel.AutoLocalize = false
    WindowTitleLabel.Parent = Sidebar
    RegisterElement(WindowTitleLabel, "TextColor3", "Text")

    local SubTitleLabel = Instance.new("TextLabel")
    SubTitleLabel.Name = "WindowSubTitle"
    SubTitleLabel.Size = UDim2.new(1, -16, 0, 12)
    SubTitleLabel.Position = UDim2.new(0, 8, 0, 26)
    SubTitleLabel.Text = subtitle ~= "" and subtitle or "by tosix"
    SubTitleLabel.TextSize = 10
    SetInterFont(SubTitleLabel, "Regular")
    SubTitleLabel.TextXAlignment = Enum.TextXAlignment.Center
    SubTitleLabel.TextTruncate = Enum.TextTruncate.AtEnd
    SubTitleLabel.BackgroundTransparency = 1
    SubTitleLabel.ZIndex = 13
    SubTitleLabel.AutoLocalize = false
    SubTitleLabel.Parent = Sidebar
    RegisterElement(SubTitleLabel, "TextColor3", "SubText")

    local ProfileCard = Instance.new("Frame")
    ProfileCard.Name = "ProfileCard"
    ProfileCard.Size = UDim2.new(1, -16, 0, 138)
    ProfileCard.Position = UDim2.new(0, 8, 0, 44)
    ...
    HubImgLabel.Size = UDim2.new(0, 60, 0, 60)
    HubImgLabel.AnchorPoint = Vector2.new(0.5, 0)
    HubImgLabel.Position = UDim2.new(0.5, 0, 0, 8)
    ...
    HubNameLabel.Position = UDim2.new(0, 6, 0, 72)
    ...
    HubSubLabel.Position = UDim2.new(0, 6, 0, 92)
    ...
    StatsContainer.Position = UDim2.new(0, 8, 0, 110)
    ...
    NavScroll.Size = UDim2.new(1, -12, 1, -198)
    NavScroll.Position = UDim2.new(0, 6, 0, 190)
    ...
    function WindowObject:AddTab(tabConfig)
        ...
        local iconEmoji = tabConfig.Icon or "⚔️"
        ...
        function Elements:Paragraph(options)
            ...
            function ParagraphMethods:AddTranslation(key, translations)
                Library:AddTranslation(key, translations)
                if key == titleText then
                    RegisterLocale(TitleLabel, key)
                elseif key == descText then
                    RegisterLocale(ContentLabel, key)
                end
            end
        ...
        function Elements:Slider(sliderText, options)
            ...
            ValueBox.FocusLost:Connect(function(enterPressed)
                ...
                if enterPressed then
                    ...
                    if num then
                        num = math.round(num / rounding) * rounding
            ...
            local function getDropdownDisplay()
                ...
                if isMulti then
                    ...
                    if #selectedList == 0 then 
                        ...
                        displayStr = table.concat(selectedList, ", ")
                    end
                else
        ...
        function Elements:SettingsUI()
            Elements:CreateSection("Theme Settings")
