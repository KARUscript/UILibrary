...
return function(Title, Parent, Collapsible)
	local TweenService = game:GetService("TweenService")
	local Section = {}
	...
	local function GetContentHeight()
		return Section.Layout.AbsoluteContentSize.Y
	end

	Creator.AddSignal(Section.Layout:GetPropertyChangedSignal("AbsoluteContentSize"), function()
		if not Collapsed then
			Section.Container.Size = UDim2.new(1, 0, 0, GetContentHeight())
			Section.Root.Size = UDim2.new(1, 0, 0, GetContentHeight() + 25)
		end
	end)
	...
	if Collapsible ~= false then
		...
		Creator.AddSignal(ClickArea.MouseButton1Click, function()
			...
			if Collapsed then
				...
				TweenService:Create(Section.Container, easeOut, {Size = UDim2.new(1, 0, 0, h)}):Play()
				TweenService:Create(Section.Root, easeOut, {Size = UDim2.new(1, 0, 0, h + 25)}):Play()
			end
		end)
	end
...
function TabModule:New(Title, Icon, Parent)
	...
	function Tab:AddSection(SectionTitle, Collapsible)
		...
		local SectionFrame = require(Components.Section)(SectionTitle, Tab.Container, Collapsible)
...
return function(Config)
	...
	Creator.AddSignal(GlobalSearchInput:GetPropertyChangedSignal("Text"), function()
		local q = GlobalSearchInput.Text:lower():gsub("%s+", "")
		for _, container in pairs(Window.ContainerHolder:GetDescendants()) do
			if container:IsA("Frame") and container.Name == "" then
				local lbl = container:FindFirstChildWhichIsA("TextLabel")
				if lbl and lbl.Text ~= "" then
					if q == "" then
						container.Visible = true
					else
						container.Visible = lbl.Text:lower():gsub("%s+",""):find(q, 1, true) ~= nil
					end
				end
			end
		end
	...
	Creator.AddSignal(SearchInput:GetPropertyChangedSignal("Text"), function()
		...
		for _, child in pairs(DropdownScrollFrame:GetChildren()) do
			if child:IsA("TextButton") then
				...
				local t = lbl and lbl.Text:lower():gsub("%s+", "") or ""
				child.Visible = q == "" or t:find(q, 1, true) ~= nil
			end
		end
...
function Element:New(Idx, Config)
	...
	Creator.AddSignal(SliderInputBox.Focused, function()
		TweenService:Create(SliderInputBox, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(36, 42, 60)}):Play()
	end)
	Creator.AddSignal(SliderInputBox.FocusLost, function()
		TweenService:Create(SliderInputBox, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(28, 32, 44)}):Play()
		local val = tonumber(SliderInputBox.Text)
		if val then
			Slider:SetValue(val)
		else
	...
	Creator.AddSignal(UserInputService.InputChanged, function(Input)
		if Dragging and (
			...
			local now = tick()
			if now - LastUpdateT < 0.014 then return end
			LastUpdateT = now
			local SizeScale = math.clamp((Input.Position.X - SliderRail.AbsolutePosition.X) / SliderRail.AbsoluteSize.X, 0, 1)
			Slider:SetValue(Slider.Min + ((Slider.Max - Slider.Min) * SizeScale))
		end
	...
	function Slider:SetValue(Value)
		...
		if not SliderInputBox:IsFocused() then
			SliderInputBox.Text = tostring(self.Value)
		end
